...
public class SchedulerMatcherTrigger extends AbstractTrigger {
  ...
  public void jobHasFinishedJustNow() {
    ...
    if (localMatcher != null) {
      localMatcher.taskFinishedAt(now());
    }
  }
  ...
  public void markThatInExecutionQueue() {
    SchedulerMatcher localMatcher = matcher;
    if (localMatcher != null) {
      localMatcher.taskFellInExecutionQueueAt(now());
    }
  }
  ...
  private void ensureMachineIdKey() {
  ...
  static byte[] ensureExistsKeyValue(byte[] content, String topKey, String key) {
    ArrayList<String> list = new ArrayList<>();
    {
      Collections.addAll(list, new String(content, UTF_8).replaceAll("\r", "").split("\n"));
    }
    {
  ...
  private void readPatternAndPlaceFromConfig() {
